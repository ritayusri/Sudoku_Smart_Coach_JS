    ...
    $.fn.sudokuJS = function(opts) {
      ...
      boardNumbers = [];
		...
		var renderBoard = function(){
			...
			//log(board);
			var j=0;
			var htmlString = "<br><b><table> <tr><td>A</td><td>B</td><td>C</td><td>D</td><td>E</td><td>F</td><td>G</td><td>H</td><td>I</td></tr></table></b>";
			for(var i=0; i < boardSize*boardSize; i++){
				htmlString += renderBoardCell(board[i], i);

				if((i+1) % boardSize === 0) {
					j+=1;
					htmlString += "<b><span style= 'color:green;'>&nbsp;&nbsp;"+j+"</span></b><br>";//*******************************************Rita
				}
			}
			//log(htmlString);
		 ...
		 * -----------------------------------------------------------------*/
		 ...
		 var renderBoardCell = function(boardCell, id){
			...
			var candidatesArray = buildCandidatesArray(candidates);
			//var candidatesInput=buildCandidatesInput();
			...
			return "<div class='sudoku-board-cell'>" +
						...
						"<input type='text' onchange='verify_row_column("+id+");' pattern='\\d*' novalidate id='input-"+id+"' value='"+val+"'"+maxlength+" >" +
						//"<div id='input-"+id+"-candidates' class='candidates'>"+candidatesString+"</div>" +
						"<input type='text' id='exsist_can"+id+"' value='"+candidates+"' hidden >"+//hidden
						"<input type='text' id='input_can"+id+"' class='in_candidates' onchange='verify_candidates("+id+");' maxlength='9' size='9'>"+//onchange='verify_candidates("+candidatesArray+");'
					"</div>";
		};
		 ...
		 * -----------------------------------------------------------------*/
		var buildCandidatesArray = function(candidatesList){
			var s= new Array();
			for(var i=1; i<10; i++){
    ...
    // requires that a board for boardSize has already been initiated
      ...
      if (contains(DIFFICULTIES, diff)) {
		...
		$boardInputs.on("keyup", function(e){
			var $this = $(this);
			//var id = parseInt($this.attr("id").replace("input-",""));//*************************Rita
		...
		$boardInputs.on("change", function(){
			var $this = $(this);
			//var id = parseInt($this.attr("id").replace("input-",""));//*************************Rita
			//keyboardNumberInput($this, id);//*************************Rita
		});
